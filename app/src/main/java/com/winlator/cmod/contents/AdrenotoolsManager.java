...
public class AdrenotoolsManager {
    ...
    public AdrenotoolsManager(Context context) {
        ...
        installBundledCommunityDrivers();
    }
    
    // Bundled community driver packages (e.g. a PanVK build for Mali/Valhall GPUs)
    // shipped under assets/community_drivers/<folder>/{meta.json, *.so}. These use
    // the exact same on-disk layout as a manually-imported driver zip, so we just copy
    // each one straight into adrenotoolsContentDir the first time it's missing — after
    // that it behaves identically to (and is indistinguishable from) any driver the user
    // installed by hand, and shows up in the same "Graphics Driver" list for any GPU
    // vendor, not just Adreno (see enumarateInstalledDrivers()/enumerateRendererDrivers(),
    // which are already vendor-agnostic).
    private static final String[] BUNDLED_COMMUNITY_DRIVER_DIRS = {"panvk-g720"};

    public void installBundledCommunityDrivers() {
        AssetManager am = mContext.getResources().getAssets();
        for (String assetDir : BUNDLED_COMMUNITY_DRIVER_DIRS) {
            try {
                String metaPath = "community_drivers/" + assetDir + "/meta.json";
                JSONObject meta = new JSONObject(FileUtils.readString(mContext, metaPath));
                String name = meta.getString("name");
                String libraryName = meta.getString("libraryName");
                File dst = new File(adrenotoolsContentDir, name);
                if (dst.exists()) continue; // already installed (or user replaced/removed it deliberately)

                File tmpDst = new File(adrenotoolsContentDir, name + ".tmp");
                if (tmpDst.exists()) FileUtils.delete(tmpDst);
                tmpDst.mkdirs();

                for (String fileName : new String[]{"meta.json", libraryName}) {
                    try (InputStream in = am.open("community_drivers/" + assetDir + "/" + fileName)) {
                        Files.copy(in, new File(tmpDst, fileName).toPath(), StandardCopyOption.REPLACE_EXISTING);
                    }
                }
                tmpDst.renameTo(dst);
                Log.d("AdrenotoolsManager", "Installed bundled community driver: " + name);
            }
            catch (Exception e) {
                Log.e("AdrenotoolsManager", "Failed to install bundled community driver " + assetDir, e);
            }
        }
    }
