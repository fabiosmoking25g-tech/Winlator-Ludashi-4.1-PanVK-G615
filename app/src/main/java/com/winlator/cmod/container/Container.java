...
public class Container {
    ...
    // MESA_SHADER_CACHE_MAX_SIZE raised from 512MB: on the Zink (OpenGL-over-Vulkan) path
    // used by non-Adreno GPUs such as Mali, every new shader permutation has to be
    // JIT-compiled by Mesa's NIR compiler the first time it's seen; once the on-disk cache
    // fills up, older entries get evicted and start recompiling again mid-game, which is a
    // common cause of stutter that reappears after a cache was "already warm". 1GB gives
    // heavier titles more headroom before that eviction kicks in; raise further if storage
    // allows. TU_DEBUG is a Turnip(Adreno)-only flag and is a no-op on Mali's Zink path.
    public static final String DEFAULT_ENV_VARS = "WRAPPER_MAX_IMAGE_COUNT=0 ZINK_DESCRIPTORS=lazy ZINK_DEBUG=compact MESA_SHADER_CACHE_DISABLE=false MESA_SHADER_CACHE_MAX_SIZE=1024MB mesa_glthread=true WINEESYNC=1 TU_DEBUG=noconform,sysmem DXVK_HUD=devinfo,fps,frametimes,gpuload,version,api";
    ...
    // DXVK_ASYNC defaults to on: the bundled DXVK for non-Adreno GPUs is explicitly the
    // "async" patched build (dxvk-1.10.3-arm64ec-async.tzst), which compiles pipeline
    // shaders on a background thread instead of blocking the render thread the first time
    // a shader combination is seen. Without DXVK_ASYNC=1 that build behaves like a normal
    // synchronous DXVK, so the async capability it ships with was previously unused —
    // this is one of the most direct levers against the "new area/effect = stutter" pattern.
    public static final String DEFAULT_DXWRAPPERCONFIG = "version=" + DefaultVersion.DXVK + ",framerate=0,async=1,asyncCache=0" + ",vkd3dVersion=" + DefaultVersion.VKD3D + ",vkd3dLevel=12_1" + ",ddrawrapper=" + Container.DEFAULT_DDRAWRAPPER + ",csmt=3" + ",gpuName=NVIDIA GeForce GTX 480" + ",videoMemorySize=2048" + ",strict_shader_math=1" + ",OffscreenRenderingMode=fbo" + ",renderer=gl";
    ...
    public String getCPUList(boolean allowFallback) {
        return cpuList != null ? cpuList : (allowFallback ? getFallbackCPUListPerformance() : null);
    }
    ...
    // On big.LITTLE/tri-cluster SoCs (e.g. Dimensity 7300: 4x Cortex-A78 + 4x Cortex-A55),
    // scheduling translated (box64/FEXCore) threads onto the LITTLE cores is a common
    // source of frame-time spikes/stutter, since the CFS scheduler will happily migrate
    // hot JIT/Wine threads there under load. We read each core's max cpufreq from sysfs
    // (no root needed) and default 64-bit workloads to only the highest-frequency cluster
    // when a clear split exists, instead of naively using every core index 0..N-1.
    private static int[] getPerformanceCoreIndices() {
        int numProcessors = Runtime.getRuntime().availableProcessors();
        long[] maxFreqKHz = new long[numProcessors];
        long highestFreq = 0;
        boolean readOk = true;
        for (int i = 0; i < numProcessors; i++) {
            try {
                ...
                maxFreqKHz[i] = sc.nextLong();
                sc.close();
                highestFreq = Math.max(highestFreq, maxFreqKHz[i]);
            }
            catch (Exception e) { readOk = false; break; }
        }

        java.util.List<Integer> fallback = new java.util.ArrayList<>();
        for (int i = 0; i < numProcessors; i++) fallback.add(i);

        if (!readOk || highestFreq == 0) return toIntArray(fallback);

        // Only the cores within ~5% of the fastest cluster's max frequency count as
        // "performance" cores. If that ends up being every core (no real split, or a
        // single-cluster CPU) we fall back to using them all.
        java.util.List<Integer> perfCores = new java.util.ArrayList<>();
        for (int i = 0; i < numProcessors; i++) {
            if (maxFreqKHz[i] >= highestFreq * 0.95) perfCores.add(i);
        }
        return perfCores.isEmpty() || perfCores.size() == numProcessors ? toIntArray(fallback) : toIntArray(perfCores);
    }
    ...
    // 64-bit translated workloads (box64/FEXCore running the game's main/render threads)
    // default to the performance-cluster cores only, to cut down on scheduler-induced
    // stutter on 4+4 (or tri-cluster) SoCs. Users can still override this per-shortcut
    // from the existing Processor Affinity UI.
    public static String getFallbackCPUListPerformance() {
        int[] perf = getPerformanceCoreIndices();
        StringBuilder sb = new StringBuilder();
        for (int core : perf) sb.append(sb.length() > 0 ? "," : "").append(core);
        return sb.toString();
    }
